...
function deepMerge(base, update) {
    Object.keys(update).forEach(function (k) {
        if (update[k] instanceof Object && k in base) {
...
module.exports = function (RED) {
    RED.nodes.registerType("iothub-registry", function(config) {
        ...
        var node = this;
        node.connectionstring = getSecretField(node, config, 'connectionstring');
        ...
        node.on('input', async (msg) => {
            ...
            if(method === 'query') {
                ...
                let allTwins = [];
                function processPage(err, twins) {
                    if (err) {
                    ...
                    if (twins && twins.length) {
                        allTwins.push(...twins.map(t => ({...t})));
                    }
                    if (query.hasMoreResults) {
                        query.nextAsTwin(processPage);
                    } else {
                        node.send({ ...msg, payload: allTwins });
                    }
                }
                query.nextAsTwin(processPage);
                return;
            ...
            if (method === 'twin.update') {
                if (!msg.etag) {
                    node.warn('No msg.etag provided. Using "*" to bypass optimistic concurrency; concurrent updates may be overwritten.');
                }
