...
module.exports = function (RED) {
    RED.nodes.registerType("iothub-registry", function(config) {
        ...
        function showError(method, err, extra) {
            const errorMessage = `Could not perform ${method}: ${err.message}`;
            node.error(errorMessage);
            node.status({ fill: "red", text: errorMessage });
            node.send({ ...extra, payload: { error: err.message } });
        }

        function callback(method, getResult, getStatus, extra) {
            return (err, result) => {
                if (err) {
                    return showError(method, err, extra);
                }
                node.send({ ...extra, payload: getResult ? getResult(result) : result });
                node.status({ fill: "green", text: method, ...getStatus?.(result) });
            }
        }

        node.on('input', async ({ deviceId, configId, method, payload, ...msg }) => {

            /* -------------------------------- Registry -------------------------------- */

            if(method === 'statistics') {
                registry.getRegistryStatistics(callback(method, s => ({...s})));
                return;
            }
            ...
            if(method === 'configs') {
                registry.getConfigurations(callback(method, cs => cs.map(c => ({...c}))));
                return;
            }
            if(method === 'config') {
                if(!configId) {
                ...
                registry.getConfiguration(configId, callback(method, c => ({...c})));
                return;
            }
            if(method === 'config.create') {
                registry.addConfiguration({ ...payload }, callback(method));
                return;
            }
            if(method === 'config.update') {
                registry.getConfiguration(configId, function(err, config) {
                    if (err) {
                        return showError(method, err);
                    }
                    ...
                    registry.updateConfiguration(config, callback(method, c => ({...c})));
                });
                return;
            }
            if(method === 'config.apply') {
                // https://learn.microsoft.com/en-us/javascript/api/azure-iothub/configurationcontent?view=azure-node-latest
                registry.applyConfigurationContentOnDevice(deviceId, payload, callback(method));
                return;
            }
            if(method === 'config.delete') {
                registry.removeConfiguration(configId, callback(method, c => ({...c})));
                return;
            }
            ...
            if(method === 'jobs') {
                registry.listJobs(callback(method, js => js.map(j => ({...j}))));
                return;
            }
            if(method === 'job') {
                registry.getJob(msg.jobId, callback(method, j => ({...j})));
                return;
            }
            if(method === 'job.cancel') {
                registry.cancelJob(msg.jobId, callback(method));
                return;
            }
            ...
            // list devices
            if(method === 'devices') {
                registry.list(callback(method, ds => (ds.map(d => ({...d})))));
                return;
            }
            ...
            if (method === 'device.create') {
                registry.create({ deviceId, status: 'enabled', ...payload }, callback(method, d => ({ ...d })));
                return;
            } 
            if (method === 'device.delete') {
                registry.delete(deviceId, callback(method, d => ({ deviceId })));
                return;
            } 
            if (method === 'device') {
                registry.get(deviceId, callback(method, d => ({ ...d })));
                return;
            } 
            if (method === 'twin') {
                registry.getTwin(deviceId, callback(method, d => ({ ...d })));
                return;
            } 
            if (method === 'twin.update') {
                registry.updateTwin(deviceId, { ...payload }, msg.etag || '*', callback(method, d => ({ ...d })));
                return;
            }
